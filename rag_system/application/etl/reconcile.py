...
def reconcile(
    ...
    for absolute_path in absolute_paths:
        if absolute_path not in warehouse_hashes:
            new_files.append(Document.from_path(data_dir, absolute_path))
        else:
            warehouse_hash = warehouse_hashes[absolute_path]
            document = Document.from_path(data_dir, absolute_path)
            if warehouse_hash == document.hash:
                unchanged_files.append(document)
            else:
                changed_files.append(document)

    deleted_files = set(warehouse_hashes.keys()) - set(absolute_paths)
    return new_files, changed_files, unchanged_files, list(deleted_files)
